...
**Priority:** 🟡 MEDIUM - Completes Epic 3.6 validation infrastructure
...
- For each film-language combination, calculate:
  ...
  - `validation_status`: 'PASS' if `overrun_minutes ≤ buffer_minutes`, else 'FAIL'
...
- Add derived columns to validation model:
  ...
  - `is_within_buffer`: Boolean `(overrun_minutes ≤ buffer_minutes)`
...
- [ ] Review SQL against dbt style guide:
  ...
  - ref() macro for model references: ✓
...
├── models/
│   ├── intermediate/
│   │   ├── int_emotion_data_quality_checks.sql  # NEW - Primary validation model
│   │   └── schema.yml                           # MODIFY - Add validation model docs
│   └── marts/
│       ├── mart_film_emotion_summary.sql        # MODIFY - Add validation comment
│       ├── mart_emotion_peaks_smoothed.sql      # MODIFY - Add validation comment
│       ├── mart_emotion_peaks_raw.sql           # MODIFY - Add validation comment
│       ├── mart_film_emotion_timeseries.sql     # MODIFY - Add validation comment
│       ├── mart_director_emotion_profile.sql    # MODIFY - Add validation comment
│       ├── mart_cross_language_emotion_comparison.sql  # MODIFY - Add validation comment
│       ├── mart_kaggle_emotion_correlation.sql  # MODIFY - Add validation comment
│       └── mart_emotion_methodology_metrics.sql # MODIFY - Add validation comment
└── tests/
    └── generic/
        └── assert_no_failed_validations.sql     # NEW - Custom dbt test macro
```
...
```
src/transformation/
├── dbt_project.yml          # MODIFY - Add validation buffer variable (optional)
└── profiles.yml             # NO CHANGE - Existing DuckDB connection
```

### Previous Story Insights

**From Story 3.6.1 (Audit) - Baseline Metrics:**
- Total film-language combinations: 75 (22 films × multiple languages)
...
├── ref('raw', 'film_emotions')     # Source: 16,694 emotion records
└── ref('stg_kaggle_films')         # Source: 22 films with runtime metadata
...
# Output: "✓ Pass assert_no_failed_validations ... [PASS in 0.12s]"
# Exit code: 0

# Failed validation (test fails)
dbt test --models int_emotion_data_quality_checks
# Output: "✗ Fail assert_no_failed_validations ... [FAIL 1 in 0.15s]"
...
1. ✅ Validation model `int_emotion_data_quality_checks.sql` created and runs successfully
2. ✅ Quality gate dbt test passes with 100% validation pass rate (0 failures)
3. ✅ Simulated validation failure detected and reported correctly by dbt test
4. ✅ Data quality metrics visible in dbt docs (summary row with pass rate)
5. ✅ All downstream marts reference validation layer via comments
6. ✅ dbt docs screenshot captured showing validation model documentation
7. ✅ Validation model executes in < 2 seconds (performance acceptable)
8. ✅ SQL code follows dbt best practices and coding standards
9. ✅ Epic 3.6 validation infrastructure complete and self-sustaining
10. ✅ Story 3.6.6 (automated CI/CD tests) unblocked for future implementation
...
**What Was Delivered**:
1. ✅ Intermediate validation model `int_emotion_data_quality_checks.sql` created
2. ✅ Custom dbt test macro `assert_no_failed_validations` implemented
3. ✅ Quality gate tests configured in `schema.yml`
4. ✅ Validation model documented in dbt docs
5. ✅ Root cause of NULL duration issue debugged and fixed
...
- **All ACs Met:** ✗ PARTIAL (8 of 9 ACs met)
  - AC1-AC5: ✓ Fully met (validation model, tests, metrics, documentation)
  - **AC6: ✗ Not met** - Mart files missing validation reference comments
  - AC7-AC9: ✓ Fully met (test execution, performance, code quality)
...
✓ Model created successfully in 0.08s

# Quality gate test execution
dbt test --select int_emotion_data_quality_checks
✗ FAIL 9 - assert_no_failed_validations
...
**Quality Gate Behavior:** ✓ Working correctly - test properly identifies failures and blocks pipeline

### Gate Status

**Gate:** CONCERNS → docs/qa/gates/3.6.5-data-quality-validation-layer.yml
...
**✗ Changes Required** - Story owner should address before marking Done:
...
## 🚨 CRITICAL FOLLOW-UP REQUIRED (FOR SM)

**Date:** 2025-11-29
**Discovered By:** Quinn (QA Review)
**Severity:** HIGH - Impacts 44% of data visibility and all data quality warnings

### Issue Summary

During QA review, a **critical architectural flaw** was discovered in the frontend that completely defeats the purpose of the data quality validation layer created in this story:

**The Problem:**
- Frontend uses `film_id` as primary identifier for film selection and validation lookups
- **77 out of 174 film-language combinations (44%)** have `film_id = NULL` in `raw.film_emotions`
- All NULL film_id combinations are **v2 subtitle versions**
- These films are **completely hidden from the UI** (excluded by `WHERE film_id IS NOT NULL`)
- All **9 FAIL validations** are v2 films → **data quality warnings never display to users**

**Evidence:**
```sql
-- Current state in raw.film_emotions
Total film-language combinations: 174
- With film_id: 97 (56%) ✓ Visible in UI
- NULL film_id: 77 (44%) ❌ Hidden from UI

-- All FAIL validations are unreachable:
the_cat_returns_es_v2: film_id = NULL (139 min overrun) ❌
the_wind_rises_fr_v2: film_id = NULL (60 min overrun) ❌
... (7 more FAIL cases, all NULL film_id)
```

**Root Cause:**
- `film_slug` is the **correct primary identifier** (100% coverage, used by validation model)
- `film_id` is **unreliable** (only 56% coverage, NULL for all v2 subtitle versions)
- Frontend incorrectly standardized on `film_id` instead of `film_slug`

### Recommended Action: Create Story 3.6.5.1 (Emergency Fix)

**Proposed Story Title:** "Fix Frontend Film Identifier Architecture (film_id → film_slug)"

**Story Type:** 🔴 EMERGENCY - Blocks Epic 3.6 completion, impacts data transparency

**Scope:**
1. **Update `get_film_list_with_metadata()`** (data_loader.py:333)
   - Query by `film_slug` instead of `film_id`
   - Remove `WHERE film_id IS NOT NULL` filter (exposes all 174 combinations)
   - Return both `film_slug` and `language_code` to selector

2. **Update `get_validation_status()`** (data_loader.py:731)
   - Accept `film_slug` + `language_code` instead of `film_id` + `language_code`
   - Remove intermediate lookup query (query validation model directly)

3. **Update `get_film_emotion_timeseries_by_id()`** and related functions
   - Rename to `get_film_emotion_timeseries()` (remove "_by_id" suffix)
   - Accept `film_slug` + `language_code` parameters
   - Update all emotion data queries to use `film_slug`

4. **Update Spirit Archives page** (pages/1_🎬_The_Spirit_Archives.py:130-136)
   - Pass `film_slug` + `language_code` to validation function
   - Update film selector to work with `film_slug` identifier

5. **Update all other pages** using film selection (Home.py, other pages)
   - Ensure consistency across all pages

**Acceptance Criteria:**
- AC1: Film selector shows all 174 film-language combinations (up from 97)
- AC2: Data quality warnings display for all 9 FAIL validations when selected
- AC3: All v2 subtitle versions accessible in UI
- AC4: No breaking changes to existing functionality (PASS validations still work)
- AC5: Validation model queries execute successfully with film_slug

**Estimated Effort:** 1-2 days (medium complexity, touches multiple files)

**Priority:** 🔴 CRITICAL - Required to make Story 3.6.5 validation layer actually functional

**Dependencies:**
- **Blocks:** Story 3.6.5 completion (data quality warnings non-functional without this)
- **Blocks:** Any user-facing demonstration of data quality features
- **Blocks:** Epic 3.6 completion

**Impact Assessment:**
- **User Experience:** Positive (44% more data visible, warnings actually work)
- **Breaking Changes:** None (architectural improvement, not API change)
- **Performance:** Neutral (film_slug queries are equally efficient)
- **Data Quality:** Critical improvement (enables validation warning visibility)

### Technical Details for Story 3.6.5.1

**Files Requiring Changes:**
```
src/app/utils/data_loader.py
├─ get_film_list_with_metadata() (lines 333-373)
├─ get_validation_status() (lines 731-797)
├─ get_film_emotion_timeseries_by_id() (line 377+)
└─ All other *_by_id() functions

src/app/pages/1_🎬_The_Spirit_Archives.py
├─ Film selector logic (lines 84-124)
└─ Validation status call (lines 130-141)

src/app/Home.py
└─ Film selector (if uses film_id)

src/app/pages/*.py
└─ All other pages using film selection
```

**Database Evidence:**
```sql
-- film_slug coverage: 100%
SELECT COUNT(DISTINCT film_slug) FROM raw.film_emotions;
-- Result: 174 ✓

-- film_id coverage: 56%
SELECT COUNT(DISTINCT film_id) FROM raw.film_emotions WHERE film_id IS NOT NULL;
-- Result: 97 ❌

-- Validation model uses film_slug (correct!)
SELECT film_slug, language_code, validation_status
FROM main_intermediate.int_emotion_data_quality_checks
WHERE film_slug = 'the_cat_returns_es_v2';
-- Result: Works perfectly ✓
```

**SM Action Required:**
1. Review this analysis
2. Create Story 3.6.5.1 with details above
3. Prioritize as CRITICAL (blocks Epic 3.6 completion)
4. Assign to Dev for immediate implementation
5. Update Epic 3.6 roadmap to include 3.6.5.1 as emergency fix

**QA Note:** This issue does not diminish the excellent quality of Story 3.6.5's validation model implementation. The validation infrastructure is **architecturally sound** and working correctly. This is a **frontend integration issue** that should have been caught during initial Epic 3.6 planning when standardizing on identifiers.
